...
let diff = Math.floor(-Math.random() + Math.random());
...
function desenhaTabela() {
  ...
  for (i = 0; i < y; i++) {
    ...
    for (j = 0; j < x; j++) {
      ...
      estilo = development
        ? `color:${renderizaIntensidade(num + j)}`
        : `background-color:${renderizaIntensidade(num + j)}`;
      htmlTable += development
        ? `  <td style="${estilo}">
...
function renderizaIntensidade(pos) {
  ...
  return `rgb(${fireColorsPalette[intensidade[pos + addColor]].r}, ${
    fireColorsPalette[intensidade[pos + addColor]].g
  }, ${fireColorsPalette[intensidade[pos + addColor]].b})`;
}
function varreArray() {
  for (i = y + 1; i != 0; i--) {
    num = i * x;
...
function attIntensidade() {
  alturaChamas = parseFloat(document.querySelector("#rangeIntensidade").value);
  console.log("altura chamas: ", alturaChamas);
}

function attDirecao(e) {
  //direção das chamas
  switch (e) {
    case 0:
      //aleatória
